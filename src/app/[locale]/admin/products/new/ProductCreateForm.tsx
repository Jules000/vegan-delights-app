...
export default function ProductCreateForm({ subcategories }: { subcategories: any[] }) {
  ...
  const [bannerPreview, setBannerPreview] = useState<{url: string, name: string} | null>(null);

  const [state, formAction, isPending] = useActionState(createAdminProduct, null);
  ...
  return (
    <form action={formAction}>
      {state?.error && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 z-[100] w-[90%] max-w-[500px]">
          <div className="bg-red-50 border-2 border-red-500 text-red-700 px-6 py-4 rounded-xl shadow-2xl flex items-start gap-3">
             <span className="material-symbols-outlined shrink-0 mt-0.5">error</span>
             <div>
               <p className="font-black uppercase text-xs tracking-wider mb-1">Erreur de création</p>
               <p className="text-sm font-medium">{state.error}</p>
             </div>
          </div>
        </div>
      )}
      {/* Hidden inputs to pass booleans as strings for action parsing */}
      ...
      <div className="flex flex-wrap justify-between items-center gap-3 mb-8">
        <div className="flex flex-col gap-1">
          ...
          <h1 className="text-admin-forest text-4xl font-black leading-tight tracking-tight">Nouveau Produit</h1>
        </div>
      </div>
          ...
          <section className="bg-white rounded-xl shadow-sm border border-black/5 p-6">
            ...
            <div className="space-y-6">
              <label className="flex flex-col">
                ...
                <input name="nameFr" required className="w-full rounded-lg text-admin-forest focus:outline-0 focus:ring-2 focus:ring-admin-primary border border-black/10 bg-admin-cream h-14 p-4 text-base font-normal" placeholder="Ex: Pâté Végétalien"/>
              </label>
              <label className="flex flex-col">
                <p className="text-admin-forest/60 text-sm font-bold pb-2 uppercase tracking-wider">Description</p>
                <textarea name="descFr" required className="w-full rounded-lg text-admin-forest focus:outline-0 focus:ring-2 focus:ring-admin-primary border border-black/10 bg-admin-cream p-4 text-base font-normal leading-relaxed" rows={4} placeholder="Description en français..."></textarea>
              </label>
            </div>
          </section>
          ...
          <section className="bg-white rounded-xl shadow-sm border border-black/5 p-6">
            ...
            <div className="space-y-6">
              <label className="flex flex-col">
                ...
                <input name="nameEn" required className="w-full rounded-lg text-admin-forest focus:outline-0 focus:ring-2 focus:ring-admin-primary border border-black/10 bg-admin-cream h-14 p-4 text-base font-normal" placeholder="Ex: Vegan Patties"/>
              </label>
              <label className="flex flex-col">
                <p className="text-admin-forest/60 text-sm font-bold pb-2 uppercase tracking-wider">Description</p>
                <textarea name="descEn" required className="w-full rounded-lg text-admin-forest focus:outline-0 focus:ring-2 focus:ring-admin-primary border border-black/10 bg-admin-cream p-4 text-base font-normal leading-relaxed" rows={4} placeholder="English description..."></textarea>
              </label>
            </div>
          </section>
          ...
          <section className="bg-white rounded-xl shadow-sm border border-black/5 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <label className="flex flex-col">
                ...
                <select name="category" className="w-full rounded-lg text-admin-forest border border-black/10 bg-admin-cream h-14 px-4 text-base font-normal outline-none focus:ring-2 focus:ring-admin-primary">
              ...
              <label className="flex flex-col">
                ...
                <select name="subcategoryId" className="w-full rounded-lg text-admin-forest border border-black/10 bg-admin-cream h-14 px-4 text-base font-normal outline-none focus:ring-2 focus:ring-admin-primary">
          ...
          <section className="bg-white rounded-xl shadow-sm border border-black/5 p-6">
            ...
            <div className="space-y-6">
              <label className="flex flex-col">
                ...
                <input name="bannerTextFr" className="w-full rounded-lg text-admin-forest focus:outline-0 focus:ring-2 focus:ring-admin-primary border border-black/10 bg-admin-cream h-14 p-4 text-base font-normal" placeholder="Ex: Savourez notre plat signature du terroir..."/>
              </label>
              <label className="flex flex-col">
                <p className="text-admin-forest/60 text-sm font-bold pb-2 uppercase tracking-wider">Promotion Text (English)</p>
                <input name="bannerTextEn" className="w-full rounded-lg text-admin-forest focus:outline-0 focus:ring-2 focus:ring-admin-primary border border-black/10 bg-admin-cream h-14 p-4 text-base font-normal" placeholder="Ex: Savor our signature local dish..."/>
              </label>
            </div>
          </section>
          ...
          <section className="bg-white rounded-xl shadow-sm border border-black/5 p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <label className="flex flex-col">
                ...
                <input name="price" required className="w-full rounded-lg text-admin-forest border border-black/10 bg-admin-cream h-14 p-4 text-base font-normal" type="number" step="0.01" placeholder="0.00"/>
              </label>
              <label className="flex flex-col">
                <p className="text-admin-forest/60 text-sm font-bold pb-2 uppercase tracking-wider">Stock Initial</p>
                <input name="stock" required className="w-full rounded-lg text-admin-forest border border-black/10 bg-admin-cream h-14 p-4 text-base font-normal" type="number" placeholder="50"/>
              </label>
              <label className="flex flex-col col-span-2">
                <p className="text-admin-forest/60 text-sm font-bold pb-2 uppercase tracking-wider">SKU</p>
                <input name="sku" required className="w-full rounded-lg text-admin-forest border border-black/10 bg-admin-cream h-14 p-4 text-base font-normal" placeholder="Ex: VEG-001"/>
              </label>
            </div>
          </section>
              ...
              {[1, 2, 3, 4].map((i: number) => (
                <div key={i} className="bg-admin-cream p-4 rounded-lg border border-black/5 flex flex-col gap-2">
                  <div className="flex gap-2">
                    ...
                    <input name={`nutritionLabel${i}En`} className="text-xs font-bold bg-white border border-black/10 p-2 rounded w-1/2" placeholder={`Label EN ${i}`} type="text" />
                  </div>
                  <input name={`nutritionValue${i}`} className="w-full bg-white border border-black/10 p-2 rounded text-lg font-black" placeholder="Valeur (ex: 280 kcal)" type="text" />
                </div>
              ))}
            </div>
          </section>
                  ...
                  <div className={`border-2 border-dashed ${previewInfo ? 'border-transparent p-0' : 'border-admin-primary/40 group-hover:border-admin-primary p-8'} bg-admin-cream rounded-xl transition-all flex flex-col items-center justify-center min-h-[320px] overflow-hidden relative`}>
                    ...
                    {previewInfo ? (
                      ...
                      <>
                        ...
                        <p className="text-admin-forest font-bold text-center">Cliquez pour importer l'image</p>
                        <p className="text-sm text-admin-forest/40 text-center mt-2">JPEG, PNG ou WebP</p>
                        <div className="mt-6 px-6 py-2 bg-admin-primary text-white font-bold rounded-lg shadow-lg shadow-admin-primary/20 cursor-pointer">Parcourir</div>
                      </>
                    )}
                    
                    <input id="imageUpload" name="image" type="file" accept="image/*" required className="hidden" onChange={handleImageChange} />
                  </div>
                </label>
              </div>
            </section>
        ...
        <div className="max-w-[1280px] mx-auto bg-white/90 backdrop-blur-xl border border-black/5 shadow-2xl rounded-2xl p-4 flex items-center justify-end">
          <div className="flex gap-3">
            ...
            <button 
              type="submit" 
              disabled={isPending}
              className={`px-10 py-3 rounded-xl bg-admin-primary text-white font-bold shadow-xl shadow-admin-primary/30 hover:brightness-110 active:scale-95 transition-all flex items-center justify-center gap-2 ${isPending ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
